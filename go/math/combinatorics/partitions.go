...
func NewPartitionSolver() *PartitionSolver {
	return &PartitionSolver{cache: make(map[twoInts]*big.Int)}
}

// Partitions returns the number of partitions an integer has.
// https://en.wikipedia.org/wiki/Partition_function_(number_theory)
func (ps *PartitionSolver) Partitions(n int) *big.Int {
	if n <= 1 {
		return big.NewInt(1)
	}
...
func (ps *PartitionSolver) partition(n, k int) *big.Int {
